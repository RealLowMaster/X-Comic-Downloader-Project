...
function nhentaiOpenInfo(name, page, whitch, makeNewTab, updateTabIndex) {
	...
	if (makeNewTab) {
		...
		if (updateTabIndex == true) tabs[Number(tabsContainer.querySelector(`[pi="${pageId}"]`).getAttribute('ti'))].addHistory(`nhentaiOpenInfo('${name}', ${page}, ${whitch}, false, false)`)
	}
...
function nhentaiInfoType(page, whitch, makeNewTab, updateTabIndex) {
	...
	if (makeNewTab) {
		...
		if (updateTabIndex == true) tabs[Number(tabsContainer.querySelector(`[pi="${pageId}"]`).getAttribute('ti'))].addHistory(`nhentaiInfoType(${page}, ${whitch}, false, false)`)
	}
	...
	const tab = tabsContainer.querySelector(`[pi="${pageId}"]`)
	tabs[Number(tab.getAttribute('ti'))].options = whitch
	const tabArea = tab.getElementsByTagName('span')[0]
	const thisTabIndex = Number(tab.getAttribute('ti'))
	tabs[thisTabIndex].ir = true
	tabs[thisTabIndex].mp = 0
	...
	nhentai.getInfoType(types[whitch], page, (err, result) => {
		...
		tabArea.textContent = `${types[whitch]} - ${page}`
		let save, save2, html
		html = '<div class="nhentai-container">'+nhentaiSiteTopMenu
...
function nhentaiRandom(makeNewTab, updateTabIndex) {
	...
	if (makeNewTab) {
		...
		if (updateTabIndex == true) tabs[Number(tabsContainer.querySelector(`[pi="${pageId}"]`).getAttribute('ti'))].addHistory(`nhentaiRandom(false, false)`)
	}
